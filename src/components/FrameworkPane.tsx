          ...
          <div className="d-flex align-items-center">
            <div className="w-100">
              ...
              >
                <InputGroup>
                  <Form.Control
                    type={compType === "image" ? "file" : "text"} // Use file input for image, text for file
                    name={compName}
                    placeholder={
                      compPlaceholder ||
                      (compType === "image"
                        ? "Pilih gambar..."
                        : `Masukkan URL atau path ${compLabel}...`)
                    } 
                    value={
                      compType === "image"
                        ? undefined
                        : formData[compName] || ""
                    } // Value for text input, undefined for file
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      if (compType === "image" && e.target.files?.[0]) {
              ...
              {formData[compName] &&
                compType !== "image" && ( // Only show clear button for text/url inputs
                  <Button
                    variant="outline-secondary"
                    onClick={() => handleClearInput(compName, inputDetails)}
                    title="Hapus Input"
              ...
              >
                <InputGroup>
                  <Form.Control
                    type="color"
                    name={compName}
                    value={formData[compName] || "#000000"}
                    onChange={(e) =>
                      handleInputChangeWithValidation(
                        compName,
                        e.target.value,
              ...
              >
                <InputGroup>
                  <Form.Control
                    ...
                    max={compMax}
                    step={compStep}
                    value={formData[compName] || compMin || 0}
                    onChange={(e) =>
                      handleInputChangeWithValidation(
                        compName,
